...
parser.add_argument(
    ...
    "--img_path",
    type=str,
    default=f"data/train_data/images",
...
parser.add_argument(
    ...
    "--gt_path",
    type=str,
    default=f"data/train_data/labels",
    ...
    if args.csv != None:
        ...
        for gt_name in tqdm(names):
            ...
            if not os.path.exists(image_path):
                continue
            gt_, new_lab_list, img_embedding, resized_size_be_padding, image_ori_size = process(gt_name, img_name, mode)
            if gt_ is not None:
                np.savez_compressed(
